...
async function pickModel(
	...
	for (const m of catalog.data ?? []) {
		...
		if (typeof health.success_rate === "number" && health.success_rate < MIN_SUCCESS_RATE) continue;
	...
	const bands = {
		...
		STANDARD: candidates.slice(tertile, Math.max(tertile + 1, candidates.length - tertile)),
		DEEP: candidates.slice(Math.max(tertile + 1, candidates.length - tertile)),
	};
